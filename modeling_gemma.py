...
# GemmaRMSNorm is a variant of RMSNorm with a learnable scale initialized to 1.0
# Used for better performance/efficiency
class GemmaRMSNorm(nn.Module):
    ...
    def _norm(self, x):
        # Normalizes its magnitude but doesn't subtract the mean
...
"""
Ouputs sine and cosine matrices used to rotate query/key vector in attention.
Preserving relative position information in a way that works better with long sequences.
"""
class GemmaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        # Computes the inverse frequency for even i
        inv_freq = 1.0 / (self.base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).float() / self.dim))
        self.register_buffer("inv_freq", tensor=inv_freq, persistent=False)
        # The frequencies decay exponentially as dimension increases.
        # Giving each dimension a unique periodicity.

    @torch.no_grad()
    def forward(self, x, position_ids, seq_len=None):
        # x: [batch_size, num_attention_heads, seq_len, head_size]
        self.inv_freq.to(x.device)
        # Copy the inv_freq tensor fo rbatch in the sequence
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            # Matrix multiply position and frequency - gives rotation angle for each position and dimension
...
class GemmaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.down_proj = nn.Linear(self.intermediate_size, self.intermediate_size, bias=False)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        # Repeat the key and value to match the number of heads of the query
        ...
        # Concatenate all the heads together. [Batch_Size, Seq_Len_Q, Num_Heads_Q, Head_Dim] -> [Batch, Seq_len_Q, Num_Heads_Q * Head_Dim]
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # Expand masks to the embedding dimension otherwise we can't use them in torch.where
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            ...
            if position_ids.dim() == 1:
                ...
                # Create a position_ids based onthe size of the attention_mask
        ...
        def forward(
            ...
            iamge_features = self.multi_modal_projector(selected_image_feature)
