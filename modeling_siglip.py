...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.positions_embedding = nn.Embedding(self.num_positions, self.embed_dim)
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        # Add position embeddings to each patch. Each positional encoidng is a vector of size [Embed_Dim]
...
class SiglipAttention(nn.Module):
    ...
    def forrward(
        ...
        # Apply softmax row_size. attn_weights: [Batch_Size, Num_Heads, Num_Patches, Num_Patches]
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)

        # Apply dropout only during training
        attn_weights = nn.funcitonal.dropout(attn_weights, p=self.dropout, training=self.training)
